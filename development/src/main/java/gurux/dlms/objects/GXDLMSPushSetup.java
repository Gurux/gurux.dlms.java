...
public class GXDLMSPushSetup extends GXDLMSObject implements IGXDLMSBase {
    ...
    public void getPushValues(final GXDLMSClient client, final List<?> values) {
        ...
        for (Entry<GXDLMSObject, GXDLMSCaptureObject> it : pushObjectList) {
            ...
            objects.add(
                    new GXSimpleEntry<GXDLMSObject, GXDLMSCaptureObject>(obj,
                            new GXDLMSCaptureObject(
                                    it.getValue().getAttributeIndex(),
                                    it.getValue().getDataIndex())));
            client.updateValue(obj, it.getValue().getAttributeIndex(),
                    values.get(pos));
            ++pos;
        }
    }
    ...
    public String[] getNames() {
        return new String[] { "Logical Name", "Object List",
                "Send Destination And Method", "Communication Window",
                "Randomisation Start Interval", "Number Of Retries",
                "Repetition Delay" };
