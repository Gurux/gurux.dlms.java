...
public class GXDLMSAssociationLogicalName extends GXDLMSObject
    ...
    static Object getKey(IGXCryptoNotifier cryptoNotifier,
            SecuritySuite securitySuite, CertificateType certificateType,
    ...
    private byte[] replyToHlsAuthentication(final GXDLMSSettings settings,
        ...
        if (settings.getAuthentication() == Authentication.HIGH_ECDSA) {
            try {
                ...
                if (settings.getCipher().getSigningKeyPair() == null) {
                    PrivateKey key =
                            (PrivateKey) getKey(settings.getCryptoNotifier(),
                                    settings.getCipher().getSecuritySuite(),
                                    CertificateType.DIGITAL_SIGNATURE,
                                    settings.getCipher().getSystemTitle(),
                                    true);
                    PublicKey pub =
                            (PublicKey) getKey(settings.getCryptoNotifier(),
                                    settings.getCipher().getSecuritySuite(),
                                    CertificateType.DIGITAL_SIGNATURE,
                                    settings.getCipher().getSystemTitle(),
                                    false);
                    settings.getCipher()
                            .setKeyAgreementKeyPair(new KeyPair(pub, key));
                }
                ...
                if (settings.getCipher()
                    ...
                    tmp = GXAsn1Converter.toByteArray(new Object[] {
                    ...
                    tmp = GXAsn1Converter.toByteArray(new Object[] {
                            new GXAsn1Integer(signature.subArray(0, 48)),
                ...
                signature.set(tmp);
                ver.initVerify(
                        settings.getCipher().getSigningKeyPair().getPublic());
            ...
            serverChallenge = GXSecure.secure(settings, settings.getCipher(),
                    ic, settings.getStoCChallenge(), readSecret);
    ...
    private void changeHlsSecret(final GXDLMSSettings settings,
    ...
    private void removeObject(final GXDLMSSettings settings,
            final ValueEventArgs e) {
        // Remove COSEM object.
        GXDLMSObject obj =
                getObject(settings, (List<?>) e.getParameters(), false);
        ...
        // Unknown objects are not removed.
            ...
            GXDLMSObject t = objectList.findByLN(obj.getObjectType(),
                    obj.getLogicalName());
            if (t != null) {
                objectList.remove(t);
            }
    ...
    private void addObject(final GXDLMSSettings settings,
            final ValueEventArgs e) {
        // Add COSEM object.
        GXDLMSObject obj =
                getObject(settings, (List<?>) e.getParameters(), true);
        // Unknown objects are not add.
        if (obj instanceof IGXDLMSBase) {
            if (objectList.findByLN(obj.getObjectType(),
                    obj.getLogicalName()) == null) {
                objectList.add(obj);
                if (settings.isServer()) {
                    // Set default values from this LN.
                    if (obj instanceof GXDLMSAssociationLogicalName) {
                        if (obj.getLogicalName()
                                .compareTo("0.0.40.0.0.255") == 0) {
                        ...
                        ln.getXDLMSContextInfo().setMaxSendPduSize(
                        ...
                        ss.setServerSystemTitle(
                                settings.getCipher().getSystemTitle());
                        ss.setGuek(settings.getCipher().getBlockCipherKey());
                        ss.setGbek(settings.getCipher()
                                .getBroadcastBlockCipherKey());
                    ...
                    for (int pos = 0; pos != count; ++pos) {
                        if (getVersion() < 3) {
                            ...
                            list[pos] = AccessMode3
                                    .toInteger(obj.getAccess3(1 + pos));
                        }
                    }
                    ...
                    for (int pos = 0; pos != count; ++pos) {
                        if (getVersion() < 3) {
                            ...
                            list[pos] = MethodAccessMode3
                                    .toInteger(obj.getMethodAccess3(1 + pos));
                        }
                    }
    ...
    private void addUser(final GXDLMSSettings settings,
            final ValueEventArgs e) {
    ...
    private void removeUser(final GXDLMSSettings settings,
            final ValueEventArgs e) {
        List<?> tmp = (List<?>) e.getParameters();
        if (tmp == null || tmp.size() != 2) {
            e.setError(ErrorCode.READ_WRITE_DENIED);
            ...
            if (settings.getIndex() == 0) {
                ...
                if (associationStatus == AssociationStatus.ASSOCIATED) {
                    for (GXDLMSObject it : objectList) {
                        if (it != this && it
                                .getObjectType() == ObjectType.ASSOCIATION_LOGICAL_NAME) {
            ...
            for (GXDLMSObject it : objectList) {
                ...
                if (!(pos <= settings.getIndex())) {
                    if (it.getObjectType() == ObjectType.ASSOCIATION_LOGICAL_NAME) {
                        if (!multipleAssociationViews && !(it == this || it
                                .getLogicalName().equals("0.0.40.0.0.255"))) {
     ...
     */
    private GXDLMSObject getObject(final GXDLMSSettings settings,
            final List<?> item, boolean add) {
        ObjectType type =
    ...
    public final void load(final GXXmlReader reader) throws XMLStreamException {
            ...
            while (!reader.isEOF()) {
                if (reader.isStartElement()) {
                    ...
                    if (target.startsWith("GXDLMS")) {
                        ...
                        reader.read();
                        ObjectType type =
                                ObjectType.getEnum(target.substring(6));
                        if (type == null) {
                        ...
                        GXDLMSObject obj =
                                reader.getObjects().findByLN(type, ln);
                        if (obj == null) {
                        ...
                        // methodAccessRights
                        String access =
                                reader.readElementContentAsString("Access");
                        ...
                        if (access != null && access != "") {
                            ...
                            for (int pos = 0; pos != buff.length; ++pos) {
                                buff[pos] = Integer.parseInt(
                                        access.substring(4 * pos, 4 * pos + 4),
                                        16) & ~0x8000;
                            }
                            accessRights.put(obj, buff);
                        }
                        ...
                        access = reader
                                .readElementContentAsString("MethodAccess3");
                        if (access != null && access != "") {
                            buff = new int[access.length() / 4];
                            for (int pos = 0; pos != buff.length; ++pos) {
                                buff[pos] = Integer.parseInt(
        ...
        multipleAssociationViews =
                reader.readElementContentAsInt("MultipleAssociationViews") != 0;
    }
    ...
    public final void save(final GXXmlWriter writer) throws XMLStreamException {
        ...
        if (objectList != null) {
            ...
            for (GXDLMSObject it : objectList) {
                ...
                if (!(it.getObjectType() == ObjectType.ASSOCIATION_LOGICAL_NAME
                        && (it == this || it.getLogicalName()
                                .equals("0.0.40.0.0.255")))) {
                    if (multipleAssociationViews || it
                            .getObjectType() != ObjectType.ASSOCIATION_LOGICAL_NAME) {
                        ...
                        if (!accessRights.containsKey(it)) {
                            if (getVersion() < 3) {
                                for (int pos = 1; pos != ((IGXDLMSBase) it)
                                        .getAttributeCount() + 1; ++pos) {
                        ...
                        if (accessRights.containsKey(it)) {
                            ...
                            for (int pos = 0; pos != buff.length; ++pos) {
                                if (getVersion() < 3) {
                                    ...
                                    // two byte.
                                    sb.append(Integer
                                            .toHexString(0x8000 | buff[pos]));
                                }
                            }
                            if (getVersion() < 3) {
                                writer.writeElementString("Access",
                                        sb.toString());
                            } else {
                                writer.writeElementString("Access3",
                                        sb.toString());
                            }
                        }
                        if (!methodAccessRights.containsKey(it)) {
                            if (getVersion() < 3) {
                                for (int pos = 1; pos != ((IGXDLMSBase) it)
                                        .getMethodCount() + 1; ++pos) {
                                    setMethodAccess(it, pos,
                                            it.getMethodAccess(pos));
                                }
                            } else {
                                for (int pos = 1; pos != ((IGXDLMSBase) it)
                                        .getMethodCount() + 1; ++pos) {
                                    setMethodAccess3(it, pos,
                                            it.getMethodAccess3(pos));
                                }
                            }
                        }
                        if (methodAccessRights.containsKey(it)) {
                            ...
                            if (getVersion() < 3) {
                                writer.writeElementString("MethodAccess",
                                        sb.toString());
                            } else {
                                writer.writeElementString("MethodAccess3",
                                        sb.toString());
                            }
                        }
        ...
        writer.writeElementString("MultipleAssociationViews",
                multipleAssociationViews);
    }
     ...
     */
    private static int getAttributeAccess(final GXDLMSObject target,
            final int attributeIndex) {
        if (attributeIndex == 1) {
            return AccessMode.READ.getValue();
        }
        GXDLMSAttributeSettings att =
                target.getAttributes().find(attributeIndex);
        if (att != null) {
     ...
     *            Access mode.
     */
    public void setAccess(final GXDLMSObject target, final int index,
     ...
     *            Access modes.
     */
    public void setAccess(final GXDLMSObject target,
            final AccessMode[] access) {
     ...
     */
    public MethodAccessMode getMethodAccess(final GXDLMSObject target,
            final int index) {
        if (target == this || (target instanceof GXDLMSAssociationLogicalName
                && target.getLogicalName().compareTo("0.0.40.0.0.255") == 0)
        ...
        return MethodAccessMode
                .forValue(methodAccessRights.get(target)[index - 1]);
    }
     ...
     *            Method access modes.
     */
    public void setMethodAccess(final GXDLMSObject target,
            final MethodAccessMode[] access) {
     ...
     * @return Access mode.
     */
    public Set<AccessMode3> getAccess3(final GXDLMSObject target,
            final int index) {
        if (target == this || (target instanceof GXDLMSAssociationLogicalName
                && target.getLogicalName().compareTo("0.0.40.0.0.255") == 0)) {
     ...
     *            Access modes.
     */
    public void setAccess3(final GXDLMSObject target,
            final Set<AccessMode3>[] access) {
        int count = target.getAttributeCount();
        if (count < access.length) {
            throw new RuntimeException("Invalid access buffer.");
     ...
     * @return Method access mode.
     */
    public Set<MethodAccessMode3> getMethodAccess3(final GXDLMSObject target,
            final int index) {
        if (target == this || (target instanceof GXDLMSAssociationLogicalName
                && target.getLogicalName().compareTo("0.0.40.0.0.255") == 0)
    ...
    public void setMethodAccess3(final GXDLMSObject target, final int index,
        ...
        if (methodAccessRights.containsKey(target)) {
            methodAccessRights.get(target)[index - 1] =
                    MethodAccessMode3.toInteger(access);
        } else {
            int[] list = new int[target.getMethodCount()];
            Arrays.fill(list, 1);
     ...
     *            Method access modes.
     */
    public void setMethodAccess3(final GXDLMSObject target,
            final Set<MethodAccessMode3>[] access) {
        int count = target.getMethodCount();
        if (count < access.length) {
            throw new RuntimeException("Invalid access buffer.");
    ...
    public String[] getNames() {
        if (version == 0) {
            return new String[] { "Logical Name", "Object List",
                    "Associated partners Id", "Application Context Name",
                    "xDLMS Context Info", "Authentication Mechanism Name",
                    "Secret", "Association Status" };
        }
        if (version == 1) {
            return new String[] { "Logical Name", "Object List",
                    "Associated partners Id", "Application Context Name",
        ...
        return new String[] { "Logical Name", "Object List",
                "Associated partners Id", "Application Context Name",
                "xDLMS Context Info", "Authentication Mechanism Name", "Secret",
                "Association Status", "Security Setup Reference", "UserList",
                "CurrentUser" };
    }
    ...
    public String[] getMethodNames() {
        if (version > 1)
            return new String[] { "Reply to HLS authentication",
                    "Change HLS secret", "Add object", "Remove object",
