...
public class GXDLMSSapAssignment extends GXDLMSObject implements IGXDLMSBase {
	...
	public final void setSapAssignmentList(final List<Entry<Integer, String>> value) {
	...
	public byte[] invoke(final GXDLMSSettings settings, final ValueEventArgs e) {
		if (e.getIndex() == 1) {
			...
			String str;
			if (tmp.get(1) instanceof byte[]) {
				str = new String((byte[]) tmp.get(1));
			} else {
				str = tmp.get(1).toString();
			}
			if (id == 0) {
				for (Entry<Integer, String> it : sapAssignmentList) {
					if (it.getValue().compareTo(str) == 0) {
						sapAssignmentList.remove(it);
						break;
					}
				}
			} else {
				sapAssignmentList.add(new GXSimpleEntry<Integer, String>(id, str));
			}
		} else {
			...
			if (e.getValue() != null) {
				...
				for (Object item : (List<?>) e.getValue()) {
					...
					if (tmp instanceof byte[]) {
						str = GXDLMSClient.changeType((byte[]) tmp, DataType.STRING, useUtc).toString();
					} else {
						str = tmp.toString();
					}
					sapAssignmentList.add(
							new GXSimpleEntry<Integer, String>(((Number) ((List<?>) item).get(0)).intValue(), str));
				}
			}
		} else {
			e.setError(ErrorCode.READ_WRITE_DENIED);
	...
	public final void load(final GXXmlReader reader) throws XMLStreamException {
		...
		if (reader.isStartElement("SapAssignmentList", true)) {
			while (reader.isStartElement("Item", true)) {
				...
				sapAssignmentList.add(new GXSimpleEntry<Integer, String>(sap, ldn));
			}
