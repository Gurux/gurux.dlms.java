    ...
    public final void setValue(final GXDLMSSettings settings,
        ...
        if (e.getIndex() == 1) {
            ...
            for (Object it : arr) {
                ...
                item.setLogicalName(
                        GXDLMSClient
                                .changeType((byte[]) tmp.get(2),
                                        DataType.OCTET_STRING, false)
                                .toString());
                item.setScriptSelector(((Number) tmp.get(3)).intValue());
                item.setSwitchTime((GXDateTime) GXDLMSClient.changeType(
                        (byte[]) tmp.get(4), DataType.DATETIME, useUtc));
                ...
                item.setBeginDate((GXDateTime) GXDLMSClient.changeType(
                        (byte[]) tmp.get(8), DataType.DATETIME, useUtc));
                item.setEndDate((GXDateTime) GXDLMSClient.changeType(
                        (byte[]) tmp.get(9), DataType.DATETIME, useUtc));
                entries.add(item);
            }
        } else {
            e.setError(ErrorCode.READ_WRITE_DENIED);
