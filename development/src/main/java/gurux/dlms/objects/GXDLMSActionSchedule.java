...
public class GXDLMSActionSchedule extends GXDLMSObject implements IGXDLMSBase {
	...
	public final Object[] getValues() {
		...
		return new Object[] { getLogicalName(), str + " " + executedScriptSelector, getType(), getExecutionTime() };
	}
	...
	public final void setValue(final GXDLMSSettings settings, final ValueEventArgs e) {
		if (e.getIndex() == 1) {
			...
			target = (GXDLMSScriptTable) settings.getObjects().findByLN(ObjectType.SCRIPT_TABLE, ln);
			if (target == null) {
			...
			if (e.getValue() != null) {
				...
				for (Object it : (List<?>) e.getValue()) {
					GXDateTime time = (GXDateTime) GXDLMSClient.changeType((byte[]) ((List<?>) it).get(0),
					...
					GXDateTime date = (GXDateTime) GXDLMSClient.changeType((byte[]) ((List<?>) it).get(1),
	...
	public final void postLoad(final GXXmlReader reader) {
		...
		if (target != null) {
			GXDLMSScriptTable t = (GXDLMSScriptTable) reader.getObjects().findByLN(ObjectType.SCRIPT_TABLE,
