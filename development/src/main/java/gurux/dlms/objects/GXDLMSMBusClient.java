...
public class GXDLMSMBusClient extends GXDLMSObject implements IGXDLMSBase {
	...
	/**
	 * @return Provides reference to an "M-Bus master port setup" object, used to
	 *         configure an M-Bus port, each interface allowing to exchange data
	 *         with one or more M-Bus slave devices
	 */
	...
	/**
	 * @param value Provides reference to an "M-Bus master port setup" object, used
	 ...
	 * @throws IllegalBlockSizeException          Illegal block size exception.
	 */
	public final byte[][] resetAlarm(final GXDLMSClient client) throws InvalidKeyException, NoSuchAlgorithmException,
			NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
	...
	public final Object[] getValues() {
		if (getVersion() == 0) {
			return new Object[] { getLogicalName(), mBusPortReference, captureDefinition, capturePeriod, primaryAddress,
					identificationNumber, manufacturerID, dataHeaderVersion, deviceType, accessNumber, status, alarm };
		}
		return new Object[] { getLogicalName(), mBusPortReference, captureDefinition, capturePeriod, primaryAddress,
				identificationNumber, manufacturerID, dataHeaderVersion, deviceType, accessNumber, status, alarm,
				configuration, encryptionKeyStatus };
	}
	...
	public final void setValue(final GXDLMSSettings settings, final ValueEventArgs e) {
		if (e.getIndex() == 1) {
			...
			if (e.getValue() != null) {
				...
				for (Object it : (List<?>) e.getValue()) {
					captureDefinition.add(new GXSimpleEntry<String, String>(
							GXDLMSClient.changeType((byte[]) ((List<?>) it).get(0), DataType.OCTET_STRING, useUtc)
									.toString(),
							GXDLMSClient.changeType((byte[]) ((List<?>) it).get(1), DataType.OCTET_STRING, useUtc)
									.toString()));
				}
			}
			...
			if (e.getIndex() == 13) {
				...
				encryptionKeyStatus = MBusEncryptionKeyStatus.values()[((Number) e.getValue()).intValue()];
			} else {
				e.setError(ErrorCode.READ_WRITE_DENIED);
			}
		} else {
	...
	public String[] getNames() {
		if (version == 0) {
			return new String[] { "Logical Name", "MBus Port Reference", "Capture Definition", "Capture Period",
					"Primary Address", "Identification Number", "Manufacturer ID", "Version", "Device Type",
					"Access Number", "Status", "Alarm" };
		}
		return new String[] { "Logical Name", "MBus Port Reference", "Capture Definition", "Capture Period",
				"Primary Address", "Identification Number", "Manufacturer ID", "Version", "Device Type",
				"Access Number", "Status", "Alarm", "Configuration", "Encryption Key Status" };
	}

	@Override
	public String[] getMethodNames() {
		return new String[] { "Slave install", "Slave deinstall", "Capture", "Reset alarm", "Synchronize clock",
				"Data send", "Set encryption key", "Transfer key" };
	}
}