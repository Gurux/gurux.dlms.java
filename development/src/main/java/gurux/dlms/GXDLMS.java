...
abstract class GXDLMS {
	...
	public static void getLNPdu(final GXDLMSLNParameters p, final GXByteBuffer reply)
		...
		boolean ciphering = p.getCommand() != Command.AARQ && p.getCommand() != Command.AARE
				&& p.getSettings().getCipher() != null && p.getSettings().getCipher().getSecurity() != null
		...
		if (p.getCommand() == Command.AARQ) {
					...
					if (p.getSettings().getNegotiatedConformance().contains(Conformance.GENERAL_BLOCK_TRANSFER)) {
						...
						if (ciphering && p.command != Command.GENERAL_BLOCK_TRANSFER) {
							...
							if ((p.getSettings().getConnected() & ConnectionState.DLMS) == 0
									|| !(p.getSettings().getCipher().getSecurity() != Security.DIGITALLY_SIGNED && p
											.getSettings().getCipher()
											.getKeyAgreementScheme() != KeyAgreementScheme.EPHEMERAL_UNIFIED_MODEL)) {
			...
			if (ciphering && reply.size() != 0 && p.getCommand() != Command.RELEASE_REQUEST
				...
				byte[] tmp;
				if ((p.getSettings().getConnected() & ConnectionState.DLMS) == 0
						|| !(p.getSettings().getCipher().getSecurity() == Security.DIGITALLY_SIGNED || p.getSettings()
								.getCipher().getKeyAgreementScheme() != KeyAgreementScheme.EPHEMERAL_UNIFIED_MODEL)) {
					tmp = cipher0(p, reply.array());
				} else {
					tmp = cipher1(p, reply.array());
				}
	...
	private static byte[] cipher1(final GXDLMSLNParameters p, final byte[] data)
			throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException,
			InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		byte keyid = (byte) p.getSettings().getCipher().getKeyAgreementScheme().ordinal();
		byte sc = 0;
		...
		if (key == null) {
			key = (PrivateKey) getKey(p.getSettings().getCryptoNotifier(), c.getSecuritySuite(),
					CertificateType.KEY_AGREEMENT, c.getSystemTitle(), true);
			c.setKeyAgreementKeyPair(new KeyPair(pub, key));
		}
		if (pub == null) {
			pub = (PublicKey) getKey(p.getSettings().getCryptoNotifier(),
					p.getSettings().getCipher().getSecuritySuite(), CertificateType.KEY_AGREEMENT,
		...
		kdf.set(GXSecure.generateKDF(alg, z, keyDataLen, GXCommon.hexToBytes(algID), c.getSystemTitle(), tmp2.array(),
				null, null));
		Logger.getLogger(GXDLMS.class.getName()).log(Level.FINEST, "kdf: {0}", kdf);
		AesGcmParameter s = new AesGcmParameter(0x31, p.getSettings(), security, c.getSecuritySuite(), 0,
				// KDF
				kdf.subArray(0, 16),
				// Authentication key.
		...
		if (keyid == 1) {
			try {
				...
				if (key == null) {
					key = (PrivateKey) getKey(p.getSettings().getCryptoNotifier(),
							p.getSettings().getCipher().getSecuritySuite(), CertificateType.DIGITAL_SIGNATURE,
	...
	static byte[] cipher0(final GXDLMSLNParameters p, final byte[] data)
		...
		AesGcmParameter s = new AesGcmParameter(cmd, cipher.getSecurity(), cipher.getSecuritySuite(),
	...
	public static void getSNPdu(final GXDLMSSNParameters p, final GXByteBuffer reply)
		...
		if (ciphering && p.getCommand() != Command.AARQ && p.getCommand() != Command.AARE) {
			GXICipher cipher = p.getSettings().getCipher();
			AesGcmParameter s = new AesGcmParameter(getGloMessage(p.getCommand()), cipher.getSecurity(),
					cipher.getSecuritySuite(), cipher.getInvocationCounter(), cipher.getSystemTitle(),
					getBlockCipherKey(p.getSettings()), getAuthenticationKey(p.getSettings()));
	 ...
	 */
	static void getMBusData(final GXDLMSSettings settings, final GXByteBuffer buff, final GXReplyData data) {
		// L-field.
	...
	static boolean isMBusData(final GXByteBuffer buff) {
		...
		return (cmd == MBusCommand.SND_NR || cmd == MBusCommand.SND_UD2 || cmd == MBusCommand.RSP_UD);
	}
	...
	public static boolean getData(final GXDLMSSettings settings, final GXByteBuffer reply, final GXReplyData data,
		...
		if (target.getXml() != null || ((frame != 0x13 && frame != 0x3) && (frame & 0x1) != 0)) {
			return true;
		}
