...
public class GuruxDlmsPushListenerExample {
    ...
    public static void main(String[] args) {
        ...
        GXNet media = new GXNet(
                settings.interfaceType == InterfaceType.COAP ? NetworkType.UDP
                        : NetworkType.TCP,
                "localhost", settings.port);
        GXDLMSSecureNotify cl =
                new GXDLMSSecureNotify(true, 1, 1, settings.interfaceType);
        ...
        p.getPushObjectList()
                .add(new GXSimpleEntry<GXDLMSObject, GXDLMSCaptureObject>(clock,
                        new GXDLMSCaptureObject(2, 0)));
        try (GXDLMSPushListener SNServer =
                new GXDLMSPushListener(settings.port, settings.interfaceType)) {
